...
public class USNG2LatLongProcessor extends GeoEventProcessorBase {
	...
	public USNG2LatLongProcessor(GeoEventProcessorDefinition definition)
	...
	public void afterPropertiesSet() {
		usng = properties.get("usng").getValueAsString();
	...
	public GeoEvent process(GeoEvent evt) throws Exception {
		...
		String usngval = (String) evt.getField(usng);
		usngval = usngval.replace(" ", ""); //remove spaces
		String[] usngvals = { usngval };

		PeGeogcs peGeoCS = PeFactory.geogcs(4326);
		// PeNotationMgrs mgrs = new PeNotationMgrs();
		double[] pts = new double[2];
		PeNotationUsng.usng_to_geog(peGeoCS, 1, usngvals, pts);
		Point pt = new Point(pts[0], pts[1]);
		MapGeometry mapGeo = null;
		if(returnBB)
		{
			int accuracy = FindAccuracy(usngval);
			Geometry geo = findBoundingBox(usngval, accuracy, pt);
	...
	private int FindAccuracy(String usng)
	{
		...
		String dropZone = usng.substring(index);
